...
{
    public class FiveMaTrading : BaseTrading
    {
        private readonly List<EmaResult> _verySlowEmas;
        private readonly List<EmaResult> _veryFastEmas;
        ...
        public FiveMaTrading(List<ChartPrice> chartPrices)
        {
            ...
            _veryFastEmas = quotes.Use(CandlePart.Close).GetEma(6).ToList();
            _verySlowEmas = quotes.Use(CandlePart.Close).GetEma(12).ToList();
        ...
        public void RebuildStatus(ChartPrice chartPrice)
        {
            ...
            {
                var verySlowEma = _verySlowEmas.Find(chartPrice.TradingDate);
                if (verySlowEma is null || verySlowEma.Ema is null)
                {
                    return;
                }

                var veryFastEma = _veryFastEmas.Find(chartPrice.TradingDate);
                if (veryFastEma is null || veryFastEma.Ema is null)
                {
                    return;
                }

                if (veryFastEma.Ema < verySlowEma.Ema && !tradingCase.ContinueBuy)
                {
        ...
        public int BuyCondition(DateTime tradingDate, float lastClosePrice)
        {
            ...
            var verySlowEma = _verySlowEmas.Find(tradingDate);
            if (verySlowEma is null || verySlowEma.Ema is null)
            {
                return 0;
            }

            var veryFastEma = _veryFastEmas.Find(tradingDate);
            if (veryFastEma is null || veryFastEma.Ema is null)
            {
                return 0;
            }

            if (veryFastEma.Ema < verySlowEma.Ema)
            {
                return 0;
            }
