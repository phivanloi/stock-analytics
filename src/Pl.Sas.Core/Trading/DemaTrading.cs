...
{
    public class DemaTrading : BaseTrading
    {
        private readonly List<DemaResult> _slowDemas;
        private readonly List<DemaResult> _fastDemas;
        private readonly List<DemaResult> _limitEmas;
        private TradingCase tradingCase = new();
        private readonly List<RsiResult> _fastRsis;
        private readonly List<RsiResult> _slowRsis;

        public DemaTrading(List<ChartPrice> chartPrices)
        {
            var quotes = chartPrices.Select(q => q.ToQuote()).OrderBy(q => q.Date).ToList();
            _fastDemas = quotes.Use(CandlePart.Close).GetDema(20).ToList();
            _slowDemas = quotes.Use(CandlePart.Close).GetDema(30).ToList();
            _limitEmas = quotes.Use(CandlePart.Close).GetDema(50).ToList();
        ...
        public void RebuildStatus(ChartPrice chartPrice)
        {
            ...
            var slowSma = _slowDemas.Find(chartPrice.TradingDate);
            if (slowSma is null || slowSma.Dema is null)
            {
                return;
            }

            var fastSma = _fastDemas.Find(chartPrice.TradingDate);
            if (fastSma is null || fastSma.Dema is null)
            {
                return;
            }

            if (fastSma.Dema < slowSma.Dema && !tradingCase.ContinueBuy)
            {
        ...
        public int BuyCondition(DateTime tradingDate, float lastClosePrice)
        {
            ...
            if (limitSma is null || limitSma.Dema is null)
            {
                return 0;
            }

            if (lastClosePrice < limitSma.Dema)
            {
                return 0;
            }

            var slowSma = _slowDemas.Find(tradingDate);
            if (slowSma is null || slowSma.Dema is null)
            {
                return 0;
            }

            var fastSma = _fastDemas.Find(tradingDate);
            if (fastSma is null || fastSma.Dema is null)
            {
                return 0;
            }

            if (fastSma.Dema < slowSma.Dema)
            {
                return 0;
            }
